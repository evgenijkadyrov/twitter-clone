...
describe('check that user logout', () => {
	beforeEach(() => {
		cy.visit('/');
	});
	it('should logout from profile and store is clear', () => {
		const user: User = {
			name: 'white',
		...
		cy.window().its('store').invoke('getState').its('user').its('token').should('be.null', true);
	});
});
